...
library(terra) ## raster operations, focal
...
env <- c(bathymetry, slope, avg_sst, mlw_dist, avg_spm, covar_spm, expo,
        eunis_other_perc, eunis_coar_perc, eunis_sand_perc)
names(env) <- vars

##### Extract and map spatial autocorrelation in the residuals #####

### Extract residuals from the model
rast <- (env$slope / env$slope) - 1 ## Set up a blank rasterfile
...
crs(xy_resids) <- "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs" ## Assign the projection WGS84
...
writeRaster(rast, paste0(wd.out,"/SPATIAL/resids_MountsBay_m1.tif"))
...
writeRaster(rac, paste0(wd.out,"/SPATIAL/rac_MountsBay_m1.tif"))

## Extract the autocovariate to a vector and add to the presence/absence locations
rac <- terra::extract(rac, xy_resids)$focal_sum
rac[is.na(rac)] <- 0 ## remove NAs
dat.rac <- cbind(dat, rac)
write.csv(dat.rac, paste0(wd.out,"/SPATIAL/dat_rac_MountsBay_m1.csv"), row.names=F)
...
AIC(final, multivar4i.rac) ## AIC much lower for spatial model (~1000)
vif(multivar4i.rac) ## Fine

save(multivar4i.rac, file=paste0(wd.out,"/SPATIAL/multivar4i_bestModel_MountsBay_rac_m1")) 
...
### Values are all very similar so ignore the convergence warnings
...
## bathy, mlw_dist, eunis_other, eunis_sand, have much steeper effects when spatial autocorrelation included
## avg_spm and interaction between expo and eunis_other change direction of effect
## Spatial autocorrelation has greater effect on parameter estimates for Mounts Bay than Fal/Helford. This isn't surprising given how clustered all the data are at Mounts Bay. 
...
plot(resids.rac) ## Looks excellent
...
testZeroInflation(multivar4i.rac) ## No zero-inflation 

### No trend in the variance for this model, so no need to investigate further

## Spatial autocorrelation
par(mfrow=c(1,2))
testSpatialAutocorrelation(resids.rac, x=dat$lon, y=dat$lat) ## p-value not significant so spatial autocorrelation removed in the model with the spatial autocovariate
...
##### Final model #####
jpeg(paste0(wd.out,"/SPATIAL/MULTIVAR4i_results_MountsBay_m1_1.jpg"), width=600, height=400)
par(mfrow=c(2,2))
...
jpeg(paste0(wd.out,"/SPATIAL/MULTIVAR4i_results_MountsBay_interact_expoEunisOther_m1.jpg"), width=600, height=400)
...
mean(gini) ## 0.949203 (0.7242677 without spatial autocovar)
sd(gini) ## 0.009257574 (0.03190105 without spatial autocovar)
...
Rsquare <- partR2(multivar4i.rac, partvars=vars, max_level=1, # the argument max_level=1 calculates part R2 for each predictor, but not for all their combinations. We can specify this with .
                 R2_type = "marginal", nboot = 10) ## Marginal R2 refers to the variance explained by fixed effect predictors relative to the total variance in the response

Rsquare$R2   # Partial R2s Very low when random intercept of 600m grid-cell not included
write.csv(Rsquare$R2, paste0(wd.out,"/SPATIAL/Rsq_MountsBay_rac_m1.csv"), row.names=F)

library(patchwork)
p1 <- forestplot(Rsquare, type = "R2", text_size = 10)
p2 <- forestplot(Rsquare, type = "IR2", text_size = 10) ## Inclusive R2. This is SC^2 * R2_full.
p3 <- forestplot(Rsquare, type = "SC", text_size = 10) ## Structure coefficients are the correlation between a predictor and the predicted response. Squared structure coefficients indicate how much of the regression effect can be attributed to a given predictor. Also described as the contribution of a predictor to model prediction irrespective of other predictors.
