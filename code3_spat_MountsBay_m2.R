...
writeRaster(rast, paste0(wd.out,"/SPATIAL/resids_MountsBay_m2.tif"))
...
writeRaster(rac, paste0(wd.out,"/SPATIAL/rac_MountsBay_m2.tif"))
...
write.csv(dat.rac, paste0(wd.out,"/SPATIAL/dat_rac_MountsBay_m2.csv"), row.names=F)
...
save(multivar4i.rac, file=paste0(wd.out,"/SPATIAL/multivar4i_bestModel_MountsBay_rac_m2")) 
...
## bathy, slope, avg_sst, eunis_other, eunis_sand, have much steeper effects when spatial autocorrelation included
## avg_spm, eunis_coar, and interaction between bathy and avg_spm change direction of effect
## Spatial autocorrelation has greater effect on parameter estimates in Mounts Bay than Fal/Helford. This isn't surprising given how clustered all the data are at Mounts Bay. 
...
##### Final model #####
jpeg(paste0(wd.out,"/SPATIAL/MULTIVAR4i_results_MountsBay_m2_1.jpg"), width=600, height=400)
par(mfrow=c(2,2))
...
##### Interaction plots ##### 
### bathymetry:avg_spm interaction in 2nd best model
jpeg(paste0(wd.out,"/SPATIAL/MULTIVAR4i_results_MountsBay_interact_bathyAvgSPM_m2.jpg"), width=600, height=400)
interactions::interact_plot(multivar4i.rac, pred=bathymetry, modx=avg_spm, interval=T,
                            y.label="Prob. Occupied", legend.main="Avg. SPM")#, rug=T)
dev.off()

##### Calculate Gini Coefficient by cross-validation #####
...
mean(gini) ## 0.9503748 (0.7323672 without spatial autocovar)
sd(gini) ## 0.004976051 (0.02155791 without spatial autocovar)
...
write.csv(Rsquare$R2, paste0(wd.out,"/SPATIAL/Rsq_MountsBay_rac_m2.csv"), row.names=F)
...
## SCs look very similar to non-spatial model
